...
--report db 수정
ALTER TABLE TBL_REPORT DROP CONSTRAINT SYS_C008117;
ALTER TABLE TBL_REPORT DROP COLUMN RPT_TITLE;
ALTER TABLE TBL_REPORT DROP CONSTRAINT SYS_C008118;
ALTER TABLE TBL_REPORT DROP COLUMN RPT_CONTENT;
ALTER TABLE TBL_REPORT DROP COLUMN RPT_REMARK;


--업무 일지 상세 내용 테이블 생성
CREATE TABLE TBL_REPORT_DETAIL (
	RPT_DETAIL_NO NUMBER(38,0) NOT NULL,
	RPT_START_TIME DATE NOT NULL,
	RPT_END_TIME DATE NOT NULL,
	RPT_CONTENT VARCHAR2(2000) NOT NULL,
	RPT_NO NUMBER(38,0) NOT NULL
);

--pk 설정
ALTER TABLE TBL_REPORT_DETAIL
ADD CONSTRAINT TBL_REPORT_DETAIL_PK PRIMARY KEY (RPT_DETAIL_NO)
ENABLE;

--pk 자동 증가 seq 생성
CREATE SEQUENCE TBL_REPORT_DETAIL_SEQ INCREMENT BY 1 MINVALUE 0 NOCYCLE NOCACHE NOORDER ;
